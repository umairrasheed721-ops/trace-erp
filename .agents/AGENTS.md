...
12. **Courier Status Mapping — 7 Core ERP Statuses Architecture (CRITICAL)**:
   ...
   **Key Safety Rules**:
   ...
   - **Single-Source-of-Truth Status Parity (CRITICAL)**: Reports (`backend/routes/reports.js`) MUST ALWAYS strictly monitor `delivery_status` (ERP Status) directly. Command Center and Reports CANNOT HAVE DIFFERENT STATUSES for any order. Voided/Cancelled orders in Shopify (`payment_status = 'voided'` or `cancelled_at IS NOT NULL`) MUST ALWAYS be mapped directly to `delivery_status = 'Cancelled'` in DB so Command Center dropdown and Reports both reflect `Cancelled`.
