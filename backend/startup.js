...
function runEarlyStartup() {
  ...
  // One-time manual fix for TR32684 Shopify fulfillment and database status
  try {
    const db = require('./db').db;
    ...
    if (order && order.shopify_order_id && order.access_token && order.access_token !== 'PENDING') {
      ...
      // Run the Shopify cancel async in background
      (async () => {
        try {
          ...
          console.error(`🔧 [Startup Fix] Async error:`, asyncErr.message);
        }
      })();
    }
  } catch (e) {
    console.error('🔧 [Startup Fix] Failed to reset TR32684 status:', e.message);
  }
