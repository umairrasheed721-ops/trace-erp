...
if (!fs.existsSync(DB_DIR)) {
  try {
    ...
    console.error('⚠️ [Startup Migration] Failed to create database directory:', e.message);
  }
}
...
try {
  ...
  if (err.message.includes('full') || err.message.includes('disk')) {
    console.warn('⚠️ Disk full error caught. Continuing boot without full startup migrations to prevent crash loop.');
  } else {
    throw err;
  }
}
