  ...
  try {
    ...
    const ordersQuery = `
      SELECT 
        ...
        SUM(CASE WHEN LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched') THEN 1 ELSE 0 END) as intransit,
        SUM(CASE WHEN LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched') THEN price ELSE 0 END) as cash_in_transit,
  ...
  try {
    const query = `
      SELECT 
        ...
        SUM(CASE WHEN LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched') THEN 1 ELSE 0 END) as in_transit,
