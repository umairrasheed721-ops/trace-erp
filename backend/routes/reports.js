  ...
  try {
    ...
    const ordersQuery = `
      SELECT 
        ...
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'delivered' THEN price ELSE 0 END) as delivered_sale,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'delivered' THEN (cost - packaging_cost) ELSE 0 END) as pure_cgs,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) NOT IN ('pending', 'cancelled', 'booked') THEN packaging_cost ELSE 0 END) as total_packaging,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'delivered' THEN courier_fee ELSE 0 END) as delivered_courier_fee,
        SUM(courier_fee) as total_courier_fee,
        SUM(paid_amount) as payment_paid,
        
        -- Counts
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('cancelled', 'canceled', 'void', 'voided') THEN 1 ELSE 0 END) as cancelations,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('pending', 'unfulfilled', 'new', 'draft', '') OR delivery_status IS NULL THEN 1 ELSE 0 END) as pending,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('booked', 'picked up', 'unassigned', 'confirmed') THEN 1 ELSE 0 END) as booked,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('delivered', 'delivered to customer') THEN 1 ELSE 0 END) as delivered,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('return received', 'restocked', 'restock') THEN 1 ELSE 0 END) as restock,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('returned', 'returned to shipper', 'returned at merchant') THEN 1 ELSE 0 END) as missing_parcel,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto', 'self delivery', 'postex', 'leopards', 'tcs') THEN 1 ELSE 0 END) as intransit,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto', 'self delivery', 'postex', 'leopards', 'tcs') THEN price ELSE 0 END) as cash_in_transit,
        SUM(CASE WHEN (tracking_number IS NULL OR tracking_number = '') AND LOWER(COALESCE(delivery_status, '')) NOT IN ('cancelled', 'canceled') THEN 1 ELSE 0 END) as without_tracking_id,
  ...
  try {
    const query = `
      SELECT 
        ...
        SUM(CASE WHEN LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto') THEN 1 ELSE 0 END) as in_transit,
