  ...
  try {
    ...
    const ordersQuery = `
      SELECT 
        ...
        SUM(CASE WHEN delivery_status IN ('Booked', 'Picked Up', 'Unassigned', 'Confirmed') THEN 1 ELSE 0 END) as booked,
        ...
        SUM(CASE WHEN LOWER(delivery_status) NOT IN ('pending', 'confirmed', 'booked', 'picked up', 'unassigned') AND LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto') THEN 1 ELSE 0 END) as intransit,
        SUM(CASE WHEN LOWER(delivery_status) NOT IN ('pending', 'confirmed', 'booked', 'picked up', 'unassigned') AND LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto') THEN price ELSE 0 END) as cash_in_transit,
        ...
        SUM(CASE WHEN LOWER(delivery_status) LIKE '%delivered%' AND ABS(COALESCE(price, 0) - COALESCE(paid_amount, 0)) > 0.9 AND (payment_status IS NULL OR (payment_status != 'Payment Posted' AND payment_status != 'Refunded' AND payment_status != 'Returned')) THEN 1 ELSE 0 END) as delivered_payment_pending,
        SUM(CASE WHEN LOWER(delivery_status) NOT IN ('cancelled', 'returned', 'return received', 'rto') AND (cost IS NULL OR cost = 0) AND items_count > 0 THEN 1 ELSE 0 END) as cost_gaps,
        SUM(CASE WHEN LOWER(delivery_status) LIKE '%delivered%' AND ABS(COALESCE(price, 0) - COALESCE(paid_amount, 0)) > 0.9 AND (payment_status IS NULL OR (payment_status != 'Payment Posted' AND payment_status != 'Refunded' AND payment_status != 'Returned')) THEN (COALESCE(price, 0) - COALESCE(paid_amount, 0)) ELSE 0 END) as unpaid_amount,
        SUM(CASE WHEN LOWER(delivery_status) LIKE '%delivered%' AND ABS(COALESCE(price, 0) - COALESCE(paid_amount, 0)) > 0.9 AND (payment_status IS NULL OR payment_status != 'Payment Posted') AND (julianday('now') - julianday(datetime(COALESCE(status_date, order_date)))) > 10 THEN 1 ELSE 0 END) as overdue_payout_count,
  ...
  try {
    const query = `
      SELECT 
        ...
        SUM(CASE WHEN LOWER(delivery_status) NOT IN ('pending', 'confirmed', 'booked', 'picked up', 'unassigned') AND LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto') THEN 1 ELSE 0 END) as in_transit,
