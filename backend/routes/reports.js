  ...
  try {
    ...
    const ordersQuery = `
      SELECT 
        ...
        SUM(CASE WHEN LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto') THEN 1 ELSE 0 END) as intransit,
        SUM(CASE WHEN LOWER(delivery_status) IN ('shipped', 'out for delivery', 'in transit', 'attempted', 'shipper advice', 'return initiated', 'return in transit', 'reattempt requested', 'undelivered', 'dispatched', 'refused', 'delivery under review', 'failed', 'rto') THEN price ELSE 0 END) as cash_in_transit,
