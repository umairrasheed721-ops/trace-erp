  ...
  try {
    ...
    const ordersQuery = `
      SELECT 
        ...
        -- Counts (100% Direct Mirror of Command Center ERP Status)
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'cancelled' THEN 1 ELSE 0 END) as cancelations,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'pending' THEN 1 ELSE 0 END) as pending,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'booked' THEN 1 ELSE 0 END) as booked,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'delivered' THEN 1 ELSE 0 END) as delivered,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'return received' THEN 1 ELSE 0 END) as restock,
        SUM(CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'returned' THEN 1 ELSE 0 END) as missing_parcel,
        ...
        -- 📊 SURPLUS_PAYOUT: Excess cash collected from non-delivered orders OR paid_amount > price on delivered orders. Source: paid_amount, price, delivery_status
        SUM(CASE WHEN COALESCE(paid_amount, 0) > 0 AND (LOWER(COALESCE(delivery_status, '')) != 'delivered' OR COALESCE(paid_amount, 0) > COALESCE(price, 0)) THEN (COALESCE(paid_amount, 0) - CASE WHEN LOWER(COALESCE(delivery_status, '')) = 'delivered' THEN COALESCE(price, 0) ELSE 0 END) ELSE 0 END) as surplus_payout,
        SUM(CASE WHEN COALESCE(paid_amount, 0) > 0 AND (LOWER(COALESCE(delivery_status, '')) != 'delivered' OR COALESCE(paid_amount, 0) > COALESCE(price, 0)) THEN 1 ELSE 0 END) as surplus_payout_count,
