...
// TEMPORARY: POST /api/public/import-csv-temp (or /api/reviews/import-csv-temp)
// Custom CSV parser that handles quotes and newlines
...
router.post('/import-csv-temp', (req, res) => {
  const { secret } = req.query;
  if (secret !== 'TraceReviewsImportSeed') {
    return res.status(403).json({ success: false, error: 'Forbidden' });
  }

  const { csv } = req.body;
  if (!csv) {
    return res.status(400).json({ success: false, error: 'Missing csv field in body' });
  }
  ...
  try {
    ...
    for (const row of dataRows) {
      ...
      try {
        ...
        console.error('Failed to insert review in route:', err.message);
        invalid++;
      }
    }
