...
// 🔍 TEMP QUERY FOR USER
router.get('/temp-rabbi-products', async (req, res) => {
  try {
    const fs = require('fs');
    const path = require('path');
    const tenantContext = require('../tenant-context');
    
    // Find all tenants on production
    const DB_PATH = process.env.DB_PATH || '/app/data/trace_erp.db';
    ...
    for (const tenantId of tenants) {
      await tenantContext.run(tenantId, async () => {
        ...
        try {
          ...
          const hasRabbi = stores.some(s => (s.store_name || '').toLowerCase().includes('rabbi') || (s.shop_domain || '').toLowerCase().includes('rabbi'));
          const hasStore14 = stores.some(s => s.id === 14);
          
          if (hasRabbi || hasStore14) {
            let products = [];
            try {
              products = db.prepare(`
                SELECT shopify_product_id, shopify_variant_id, sku, title, price, status 
                FROM products 
                WHERE title LIKE '%Fed%' OR title LIKE '%Prey%' OR title LIKE '%Fred%' OR title LIKE '%Perry%'
              `).all();
            } catch(pe) {
              products = db.prepare(`
                SELECT shopify_product_id, shopify_variant_id, sku, title, price 
                FROM products 
                WHERE title LIKE '%Fed%' OR title LIKE '%Prey%' OR title LIKE '%Fred%' OR title LIKE '%Perry%'
              `).all();
            }
            ...
            const costs = db.prepare(`
              SELECT shopify_variant_id, parent_title, variant_title, sku, status 
              FROM product_master_costs 
              WHERE parent_title LIKE '%Fed%' OR parent_title LIKE '%Prey%' OR parent_title LIKE '%Fred%' OR parent_title LIKE '%Perry%'
            `).all();

            results.push({
              tenantId,
              stores: stores.map(s => ({ id: s.id, shop_domain: s.shop_domain, store_name: s.store_name })),
              products,
              costs
            });
          }
...
// GET /api/diagnostics/test-instaworld-proxy/:tracking
router.get('/test-instaworld-proxy/:tracking', async (req, res) => {
    try {
        const { tracking } = req.params;
        const store = db.prepare('SELECT * FROM stores LIMIT 1').get();
        if (!store) return res.status(404).json({ error: 'No store found in database' });
        ...
        const { instaworldFetch } = require('../engines/instaworld_http');
        const url = 'https://one-be.instaworld.pk/logistics/v1/trackShipment';
        console.log(`🔌 Instaworld Proxy Probe for tracking: ${tracking}...`);
        const start = Date.now();
        const response = await instaworldFetch(url, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        res.json({
            proxy_test: true,
            proxy_url: store.gas_proxy_url || 'None configured',
            status: response.status,
            duration: `${duration}ms`,
            response: body
        });
