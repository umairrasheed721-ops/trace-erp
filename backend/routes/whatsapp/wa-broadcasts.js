...
router.post('/settings', (req, res) => {
  ...
  try {
    ...
    `).run(mode, cod_verification_enabled ? 1 : 0, attempted_delivery_enabled ? 1 : 0, dispatch_alerts_enabled ? 1 : 0, enable_cod_reminders !== undefined ? (enable_cod_reminders ? 1 : 0) : 1, Number(min_delay_sec), Number(max_delay_sec), Number(max_per_hour), Number(cooling_period_min), cod_template, attempted_template, dispatch_template, ai_responder_enabled ? 1 : 0, ai_tracking_template || '', ai_landmark_template || '', pollOptionsStr);
