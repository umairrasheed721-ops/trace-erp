  ...
  try {
    ...
    db.prepare(`
      UPDATE whatsapp_settings SET
        mode = ?, cod_verification_enabled = ?, attempted_delivery_enabled = ?, dispatch_alerts_enabled = ?, enable_cod_reminders = ?, enable_post_delivery_feedback = ?, post_delivery_template = ?, min_delay_sec = ?, max_delay_sec = ?, max_per_hour = ?, cooling_period_min = ?, cod_template = ?, attempted_template = ?, dispatch_template = ?, ai_responder_enabled = ?, ai_tracking_template = ?, ai_landmark_template = ?, poll_options = ?, auto_responders = ?, enable_manual_chat_dispatch = ?, enable_automated_broadcasts = ?, vip_bypass_manual = ?, ephemeral_mode = ?, updated_at = datetime('now')
    `).run(
      mode,
      ...
      ephemeral_mode !== undefined ? (ephemeral_mode ? 1 : 0) : 0
    );

    // Update bot in memory
    bot.setSettings({ minDelaySec: min_delay_sec, maxDelaySec: max_per_hour, maxPerHour: max_per_hour, coolingPeriodMin: cooling_period_min, aiResponderEnabled: ai_responder_enabled ? 1 : 0, aiTrackingTemplate: ai_tracking_template, aiLandmarkTemplate: ai_landmark_template, ephemeralMode: ephemeral_mode !== undefined ? (ephemeral_mode ? 1 : 0) : 0 });
